...
Onboarding establishes **who the merchant is** and **what they are allowed to accept**. The counterparties are the merchant and an **acquirer** (often through a **gateway / PSP** integration layer). In standard card-acquirer onboarding, **the shopper is not involved** — onboarding sets the boundaries for what the merchant can later initiate at checkout.

For cards, onboarding is anchored by **scheme acceptance rules** and **PCI program obligations**: branding, MCC (Merchant Category Code), permitted products, and cardholder-data handling expectations are enforced through the acquirer on sponsored merchants. For **LPMs** there is no single scheme analog. Requirements differ by **bank**, **wallet operator**, or **platform**, and even the same rail can vary by country or tier.
...
sequenceDiagram
    ...
    Acq->>Mer: MID, pricing, cleared to accept the brand
```

A successful onboarding typically yields a **MID** (Merchant ID):
- Card onboarding often produces a **hierarchy** of identifiers — in common setups this may include one or more MIDs (for example by scheme, currency, or legal entity), with **store IDs** and **terminal IDs (TID)** beneath them.
...
- When a PSP aggregates acquirers on your behalf, the **PSP-level account ID** is the primary identifier the merchant receives from the PSP; the underlying acquirer MIDs are managed by the PSP and may not be directly visible to you.
...
- `Approved`: underwriting passed for the full requested scope (brands, MCCs, currencies, geographies).
...
- `Rejected`: underwriting refused and no MID is produced. Usually terminal for this application; some providers support appeal or reopen paths, while others require a new application.
- `Active`: MID is provisioned and the merchant is cleared to submit transactions within the approved scope.
- `Suspended`: a previously active MID has been paused: voluntarily, by dormancy policy, or due to risk / compliance review. Typically no new authorizations are accepted; in-flight obligations (settlement, disputes, refunds) continue.
...
stateDiagram-v2
    ...
    Approved --> Active: MID provisioned / cleared to transact
    PartiallyApproved --> Active: MID provisioned / cleared to transact
...
- **Semantics**: answer one question: *"May this merchant submit transactions of type X, in country Y, on rail Z, from date D?"* Output is a credentialed identity and a scope (brands, MCCs, currencies, volumes, entity).
...
- **Time discipline**: review SLAs are bounded for cards at PSPs that publish one (hours to a few days) and mostly unbounded for manually handled LPMs. **Document validity** windows apply, so long-paused applications force re-collection. Some programs require periodic re-validation, including annual PCI evidence cycles and, in specific cases, scheme-program renewals.
